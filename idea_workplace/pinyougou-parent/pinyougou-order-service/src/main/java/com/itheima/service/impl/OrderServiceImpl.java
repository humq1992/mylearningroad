...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		//业务需求每一个商家对应一个order   所有的order对应一个payLog


		//获得redis中再cartservice中存入的购物车列表
		List<Cart> cartList = (List<Cart>) redisTemplate.boundHashOps("cartList").get(order.getUserId());
		//循环遍历集合 用雪花算法为其中添加orderid seller id等属性
		//遍历每一个cart对象中的orderitemlist对象获得每一个orderitem对象

		//在redis中余留一个订单集合对象，用来获得所有订单总计的费用
