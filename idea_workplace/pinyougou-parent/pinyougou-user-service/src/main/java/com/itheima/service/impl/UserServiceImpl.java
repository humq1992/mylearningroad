...
//如果有外部配置可以使用下面两项 来注入外部属性在map赋值时直接使用
	/*
	@Value("${template_code}")
	...
	*/
	...
	public void creatSmsCode(String phone) {
	 final	String code=(long)Math.random()*1000000+"";
		...
		//在这里存储时候采用hashmap存储 存储的文件名为smscode  key为phone 以phone存入code便于
		//之后调用phone来查询code值发短信验证；
